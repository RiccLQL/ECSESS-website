<template>
    <div class="flex-vertical">
        <div class="grid">
            <div class="flex-vertical centered cell" v-for="(cell, index) in gridCells" :key="index">
                <div class="cell-picture flex-vertical">
                    <Picture :alt="cell.image.alt" :path="cell.image.path" :size="gridImageSize" />
                </div>
                <div>
                    <h4>{{cell.title}}</h4>
                    <TextArea :text="cell.description" />
                    <Button v-if="cell.button" :clickParams="cell.buttonLink" :color="buttonColor" :size="buttonSize" :text="cell.button" @handleClick="goToLink"/>
                </div>
            </div>
        </div>
    </div>
